...
datatype exp =
  Evar var
  | Elit val
  | Ebinop exp "val \<Rightarrow> val \<Rightarrow> val" exp
...
primrec edenot :: "exp \<Rightarrow> store \<Rightarrow> nat"
  where
  "edenot (Evar v) s = s v"
| "edenot (Elit n) s = n"
...
where
  ...
  | "accesses (C1;; C2)     s = accesses C1 s"
