...
definition exhale_perm_single :: "mask \<Rightarrow> heap_loc \<Rightarrow> prat option \<Rightarrow> mask set"
  where "exhale_perm_single m l p_opt =
      {m'| m' q. 
               (p_opt = None \<longrightarrow> pgt q pnone) \<and>
               option_fold ((=) q) (q \<noteq> pnone \<and> pgt (m l) q) p_opt \<and>
               m' = m(l := psub (m l) q)
       }"
...
\<comment>\<open>exhale acc(e.f, p)\<close>
    ...
    "\<lbrakk> \<omega> = update_mask_total_full \<omega>0 m;
       Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e_r; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VRef r); 
       Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e_p; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VPerm p);
...
\<comment>\<open>exhale acc(P(es), p)\<close>
  | ExhAccPred:
     "\<lbrakk> \<omega> = update_mask_total_full \<omega>0 m;
       ...
       Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e_p; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VPerm p) \<rbrakk> \<Longrightarrow>
...
\<comment>\<open>exhale other cases\<close>
  | ExhPure:
    "\<lbrakk> \<omega> = update_mask_total_full \<omega>0 m; 
       Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool b) \<rbrakk> \<Longrightarrow>
  ...
  | SubAtomicFailure: 
    "\<lbrakk> e \<in> sub_expressions_atomic A ;
      Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e; \<omega>\<rangle> [\<Down>]\<^sub>t VFailure  \<rbrakk> \<Longrightarrow> 
...
\<comment>\<open>exhale A \<longrightarrow> B\<close>
 | ExhImpTrue: 
   "\<lbrakk>  \<omega> = update_mask_and_pmask_total_full \<omega>0 (fst m_pm) (snd m_pm);
      Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool True); 
      red_exhale Pr \<Delta> \<omega>0 A m_pm res \<rbrakk> \<Longrightarrow>
      red_exhale Pr \<Delta> \<omega>0 (Imp e A) m_pm res" 
 | ExhImpFalse:  
   "\<lbrakk> \<omega> = update_mask_and_pmask_total_full \<omega>0 (fst m_pm) (snd m_pm);
      Pr, \<Delta>, Some (get_valid_locs \<omega>0) \<turnstile> \<langle>e; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool False) \<rbrakk> \<Longrightarrow> 
...
*)
definition havoc_undef_locs :: "'a total_heap \<Rightarrow> 'a predicate_heap \<Rightarrow> mask \<Rightarrow> 'a predicate_mask \<Rightarrow> 'a total_heap set"
  where "havoc_undef_locs h m pm = {h' | h'. \<forall>l. pgt (m l) pnone \<longrightarrow> h' l = h l}"
