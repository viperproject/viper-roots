...
ML \<open>
  ...
  fun red_assume_good_state_tac ctxt ctxt_wf_thm tr_def_thms = 
    resolve_tac ctxt [@{thm RedAssumeOk}] THEN'
    resolve_tac ctxt [@{thm assume_state_normal} OF [ctxt_wf_thm]] THEN'
    fastforce_tac ctxt [] THEN'
    assm_full_simp_solved_with_thms_tac tr_def_thms ctxt THEN'
    assm_full_simp_solved_with_thms_tac tr_def_thms ctxt THEN'
    assm_full_simp_solved_with_thms_tac tr_def_thms ctxt THEN'
    assm_full_simp_solved_with_thms_tac tr_def_thms ctxt
  
  fun progress_assume_good_state_tac ctxt ctxt_wf_thm tr_def_thms =
    ...
    red_assume_good_state_tac ctxt ctxt_wf_thm tr_def_thms THEN'
    assume_tac ctxt THEN'
    resolve_tac ctxt [@{thm conjI}] THEN'
    resolve_tac ctxt [@{thm red_ast_bpl_refl}] THEN'
    assume_tac ctxt

  fun progress_assume_good_state_rel_tac ctxt ctxt_wf_thm tr_def_thms =
