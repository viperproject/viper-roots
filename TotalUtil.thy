...
fun update_store_total :: "'a full_total_state \<Rightarrow> var \<Rightarrow> 'a val \<Rightarrow> 'a full_total_state"
  where "update_store_total \<omega> x v = ((get_store_total \<omega>)(x \<mapsto> v), get_trace_total \<omega>, get_mh_total_full \<omega>)"

lemma update_store_total_trace_same: "get_trace_total (update_store_total \<omega> x v) = get_trace_total \<omega>"
  by simp

lemma update_store_total_hm_same: "get_mh_total_full (update_store_total \<omega> x v) = get_mh_total_full \<omega>"
  by simp
...
fun update_trace_total :: "'a full_total_state \<Rightarrow> 'a total_trace \<Rightarrow> 'a full_total_state"
  where "update_trace_total \<omega> \<pi> = (get_store_total \<omega>, \<pi>, get_mh_total_full \<omega>)"

lemma update_trace_total_store_same: "get_store_total (update_trace_total \<omega> \<pi>) = get_store_total \<omega>"
  by simp

lemma update_trace_total_hm_same: "get_mh_total_full (update_trace_total \<omega> \<pi>) = get_mh_total_full \<omega>"
  by simp
...
definition update_heap_total :: "'a total_state \<Rightarrow> heap_loc \<Rightarrow> 'a val \<Rightarrow> 'a total_state"
  where "update_heap_total \<phi> l v = Abs_total_state ( ((get_mask_total \<phi>), (get_heap_total \<phi>)(l := v)), get_pmh_total \<phi>)"

definition update_mask_total :: "'a total_state \<Rightarrow> mask \<Rightarrow> 'a total_state"
  where "update_mask_total \<phi> m' = Abs_total_state ((m', get_heap_total \<phi>), get_pmh_total \<phi>)"
...
fun update_heap_total_full :: "'a full_total_state \<Rightarrow> heap_loc \<Rightarrow> 'a val \<Rightarrow> 'a full_total_state"
  where "update_heap_total_full \<omega> l v = (get_store_total \<omega>, get_trace_total \<omega>, update_heap_total (get_mh_total_full \<omega>) l v )"

lemma update_heap_total_full_lookup_1: "get_heap_total_full (update_heap_total_full \<phi> l v) l = v"
  by (metis get_heap_total_full.simps get_mh_total_full.simps snd_conv update_heap_total_full.simps update_heap_total_lookup_1)
...
lemma update_heap_total_full_lookup_2: "l1 \<noteq> l2 \<Longrightarrow> get_heap_total_full (update_heap_total_full \<phi> l1 v) l2 = get_heap_total_full \<phi> l2"
  by (metis get_heap_total_full.simps get_mh_total_full.simps snd_conv update_heap_total_full.simps update_heap_total_lookup_2)
...
lemma update_heap_total_overwrite_full: "update_heap_total_full (update_heap_total_full \<omega> l v1) l v2 = update_heap_total_full \<omega> l v2"
 ...
 (* apply (rule full_total_state_eq)
  ...
  by (metis get_mh_total_full.elims get_mask_total_full.elims snd_conv update_heap_total_full.simps update_heap_total_mask_same)*)
  
fun update_mask_total_full :: "'a full_total_state \<Rightarrow> mask \<Rightarrow> 'a full_total_state"
  where "update_mask_total_full \<omega> m = (get_store_total \<omega>, get_trace_total \<omega>, update_mask_total (get_mh_total_full \<omega>) m)"

lemma update_heap_total_full_mask_same: "get_mask_total_full (update_heap_total_full \<omega> l v) = get_mask_total_full \<omega>"
  by (metis get_mh_total_full.simps get_mask_total_full.simps snd_conv update_heap_total_full.simps update_heap_total_mask_same)
